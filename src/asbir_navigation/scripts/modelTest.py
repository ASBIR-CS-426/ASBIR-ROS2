...
from geometry_msgs.msg import Point, Quaternion, Transform, TransformStamped, Pose, Vector3, Vector3Stamped, PointStamped
...
class ModelTest(Node):
    def __init__(self):
            ...
            # self.pointPub = self.create_publisher(PointStamped, 'point_to_transform')
            # self.pointSub = self.create_suscriber(PointStamped, "transformed_point", transformedCallback)
    ...
    def pathModel(self):
        ...
        for j in self.points:            
            waiting = True
            transformPoint = j.frame_pos
            transformPoint.child_frame_id = j.frame_pos.child_frame_id
            self.tfPointStaticBroadcaster.sendTransform(transformPoint)
            # j.pos = self.tfBuffer.lookup_transform('temp', "odom_frame", Duration(), Duration())
            j.pos = self.tfBuffer.lookup_transform('odom_frame', "temp", Duration(), Duration())
            waiting = False
            vert=j.id
            edges = []
