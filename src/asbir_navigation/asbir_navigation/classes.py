#!/usr/bin/env python3
import rclpy
from geometry_msgs.msg import TransformStamped, Point, Quaternion, Pose, PointStamped, PoseStamped, Vector3
import numpy as np
...
class Surface:
	...
	def __init__(self, id='odom_frame', xMin=0, xMax=0, yMin=0, yMax=0, zMin=0, zMax=0, xDim=0, yDim=0):
		self.id = id
		self.xMin = xMin
		self.xMax = xMax
		self.yMin = yMin
...
class Vertice:
	def __init__(self, frame_pos = TransformStamped(), surface = Surface()):
		self.id = ''
...
class FindEdge:
    ...
    def getEdge(self,source,target,dist,tfBuffer):
        ...
        if target_frame != source_frame:
            target_pos = tfBuffer.transform(target_pos, source_frame, rclpy.duration.Duration(10))
        ...
        # q = tfBuffer.transform(q, 'camera_odom_frame', rclpy.duration.Duration(10))
        q = tfBuffer.transform(q, 'odom_frame', rclpy.duration.Duration(10))
